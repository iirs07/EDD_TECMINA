...
export default function EjercicioScreen() {
  ...
  const [text, setText] = useState<string[]>([
    '______',
    'Se accede a los elementos de forma secuencial.',
    '______',
    'Se pueden recorrer con bucles fácilmente.',
  ]);

  const emptySpacesOriginal = [0, 2];
  const [options, setOptions] = useState<string[]>([
    'Facil Recorrido',
    'Acceso Ordenado',
  ]);
  const [emptySpaces, setEmptySpaces] = useState<number[]>([0, 2]);
  ...
  const checkAnswer = () => {
    if (selectedWords[0] === 'Acceso Ordenado' && selectedWords[1] === 'Facil Recorrido') {
      setIsCorrect(true);
    } else {
      setIsCorrect(false);
    }
  ...
  const resetText = () => {
    setText(['______', 'Se accede a los elementos de forma secuencial.', '______', 'Se pueden recorrer con bucles fácilmente.']);
    setEmptySpaces([0, 2]);
    setOptions(['Facil Recorrido', 'Acceso Ordenado']);
  ...
  return (
    <SafeAreaView style={styles.container}>
      <View style={styles.blueBar}>
        ...
        <Text style={styles.blueBarText}>🧠 Ejercicio 3</Text>
      </View>
        ...
        <View style={styles.optionsContainer}>
          {options.map((word, index) => (
            <TouchableOpacity 
              ...
              style={[styles.optionButton, { width: width * 0.42 }]}
            >
...
const styles = StyleSheet.create({
  ...
  text: { fontSize: 20, lineHeight: 30, marginBottom: 10 },
  normalText: { color: '#333' },
  spaceText: { color: '#999', fontWeight: 'bold', letterSpacing: 2 },
