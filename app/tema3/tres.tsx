...
export default function EjercicioScreen() {
  ...
  const [text, setText] = useState<string[]>([
    'Los elementos están organizados en un orden específico.',
    '______',
  ...
  const resetText = () => {
    ...
    setEmptySpaces([1, 3]);
    setOptions(['Secuencialidad', 'Relación Directa']);
  ...
  const handleNext = () => {
    if (isCorrect === null) {
      if (selectedWords.includes(null)) return; // No verificar si falta llenar espacios
      if (selectedWords[0] === 'Secuencialidad' && selectedWords[1] === 'Relación Directa') {
        setIsCorrect(true);
      } else {
        setIsCorrect(false);
      }
    } else if (isCorrect === true) {
      router.push('/tema3/cuatro');
    } else {
      resetText();
    }
  };
  ...
  return (
    <SafeAreaView style={styles.container}>
      <View style={styles.blueBar}>
        ...
        <Text style={styles.blueBarText}>🧠 Ejercicio 2</Text>
      </View>
...
const styles = StyleSheet.create({
  ...
  text: { fontSize: 19, lineHeight: 30, marginBottom: 10 },
