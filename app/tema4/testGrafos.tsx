...
const Grafos: React.FC = () => {
  ...
  const [showMenuButton, setShowMenuButton] = useState<boolean>(false); // Nuevo estado para el botón "Ir a Grafos"

  const respuestasCorrectas: SelectedOption = {
    pregunta1: 'Conjunto de vertices y arcos.',
    pregunta2: 'Cada nodo está conectado con todos los demás nodos.',
    pregunta3: 'Un grafo nulo.',
    pregunta4: 'Un camino que une dos nodos cualesquiera.',
  };
  ...
  const comprobarRespuestas = () => {
    ...
    if (
      ...
      respuestasCorrectasCount++;
    if (
        selectedOption.pregunta4 &&
        normalizar(selectedOption.pregunta4) === normalizar(respuestasCorrectas.pregunta4!)
    )
        respuestasCorrectasCount++;

    setFeedback(
      respuestasCorrectasCount === 4
        ? '🎉 ¡Felicidades, todas tus respuestas son correctas!'
        : '❌ ¡Inténtalo de nuevo!'
    );

    // Mostrar el botón "Ir a Grafos" si todas las respuestas son correctas
    if (respuestasCorrectasCount === 4) {
  ...
  return (
    <SafeAreaView style={styles.safeArea}>
      ...
      <View style={styles.blueBar}>
        <TouchableOpacity onPress={() => router.push('/tema4/SimuladorGrafos')} style={styles.backButton}>
      ...
      <ScrollView style={styles.container}>
        <Text style={styles.titulo}>Prueba sobre Grafos</Text>

        {/* Pregunta 1 */}
        <View style={styles.card}>
          <Text style={styles.subtitulo}>¿Qué es un grafo?</Text>
          {[
      'Conjunto de vertices y arcos.',
      'Una estructura que representa relaciones entre objetos.',
      'Un tipo de arreglo bidimensional.',
      'Un tipo de árbol binario.'
        ...
        {/* Pregunta 2 */}
        <View style={styles.card}>
          <Text style={styles.subtitulo}>¿Qué significa que un grafo sea completo?</Text>
          {['Todos los nodos tienen el mismo valor.',
      'Hay ciclos en todos los nodos.',
      'Cada nodo está conectado con todos los demás nodos.',
      'No tiene conexiones entre nodos.'
].map((opcion) => (
            <TouchableOpacity
              key={opcion}
        ...
        {/* Pregunta 3 */}
        <View style={styles.card}>
          <Text style={styles.subtitulo}>¿A que se le conoce como Grafo de orden 0?</Text>
          {[
            'Un grafo nulo.',
      'Un conjunto de nodos.',
      'Un camino de un nodo.',
      'Una ruta entre dos nodos .'
        ...
        {/* Pregunta 4 */}
        <View style={styles.card}>
          <Text style={styles.subtitulo}>¿Que es un grafo conectado?</Text>
          {[
            'Es la distancia al nodo raíz.',
            'Un camino que une dos nodos cualesquiera.',
            'Es la posición de un nodo en el árbol.'
          ].map((opcion) => (
            <TouchableOpacity
              key={opcion}
              style={[styles.optionButton, selectedOption.pregunta4 === opcion && styles.selectedOption]}
              onPress={() => handleOptionSelect(opcion, 'pregunta4')}
            >
              <Text style={styles.optionText}>{opcion}</Text>
            </TouchableOpacity>
          ))}
