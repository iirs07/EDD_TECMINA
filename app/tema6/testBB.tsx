...
const TestBB: React.FC = () => {
  ...
  const respuestasCorrectas: SelectedOption = {
    pregunta1: 'En reducir en cada paso el ámbito de búsqueda a la mitad de elementos, basandose en comparar el elemnto a buscar con el elemento que se encuentra a la mitad del intervalo.',
    pregunta2: 'Izquierda',
    pregunta3: 'Cuando el vector esta ordenado o puede ordenarse.',
  ...
  return (
    <SafeAreaView style={styles.safeArea}>
      ...
      <View style={styles.blueBar}>
        <TouchableOpacity onPress={() => router.push('/tema6/simuladorBB')} style={styles.backButton}>
      ...
      <ScrollView style={styles.container}>
        <Text style={styles.titulo}>Prueba Busqueda Binaria</Text>

         {/* Pregunta 1 */}
      <View style={styles.card}>
        <Text style={styles.subtitulo}>¿En qué consiste la Busqueda Binaria?</Text>
        {['En reducir en cada paso el ámbito de búsqueda a la mitad de elementos, basandose en comparar el elemnto a buscar con el elemento que se encuentra a la mitad del intervalo.', 'En buscar secuencialmente el valor buscado hasta encontrarlo.', 'Ordenar el arreglo de datos.'].map(opcion => (
          <TouchableOpacity
            key={opcion}
      ...
      {/* Pregunta 2 */}
      <View style={styles.card}>
        <Text style={styles.subtitulo}>En caso de que el elemento buscado sea menor al valor medio, ¿En que lado de la matriz se continua buscando?</Text>
        {['Izquierda', 'Derecha', 'Mitad'].map(opcion => (
          <TouchableOpacity
            key={opcion}
      ...
      {/* Pregunta 3 */}
      <View style={styles.card}>
        <Text style={styles.subtitulo}>¿Cuando es recomendable usar la Busqueda Binaria?</Text>
        {['Cuando el vector esta ordenado o puede ordenarse.', 'Cuando es secuencial.', 'Cuando no este ordenado.'].map(opcion => (
          <TouchableOpacity
            key={opcion}
