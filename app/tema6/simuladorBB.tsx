...
export default function App() {
  ...
  const [mensajeExplicacion, setMensajeExplicacion] = useState('');
  ...
  const agregarValor = () => {
    ...
    if (!isNaN(numero) && lista.length < MAX_VALORES) {
      const nuevaLista = [...lista, numero].sort((a, b) => a - b);
      setLista(nuevaLista);
      setNuevoValor('');
      setEncontrado(null);
      setVisitandoIndice(null);
      setBusquedaFinalizada(false);
      setMensajeExplicacion('');
    }
  };

  const delay = (ms: number) => new Promise(resolve => setTimeout(resolve, ms));

  const buscarBinaria = async () => {
    ...
    setEncontrado(null);
    setBusquedaFinalizada(false);
    setMensajeExplicacion('Iniciando búsqueda...');
    ...
    while (izquierda <= derecha) {
      ...
      setVisitandoIndice(medio);

      if (lista[medio] === valor) {
        setMensajeExplicacion(`¡Número encontrado en índice ${medio}!`);
        setEncontrado(medio);
        break;
      } else if (lista[medio] < valor) {
        setMensajeExplicacion(`Valor buscado es mayor que ${lista[medio]}. Buscando en la mitad derecha (índices ${medio + 1} a ${derecha}).`);
        izquierda = medio + 1;
      } else {
        setMensajeExplicacion(`Valor buscado es menor que ${lista[medio]}. Buscando en la mitad izquierda (índices ${izquierda} a ${medio - 1}).`);
        derecha = medio - 1;
      }
    ...
    if (izquierda > derecha && encontrado === null) {
      setMensajeExplicacion('Número no encontrado en la lista.');
    }
  ...
  return (
    <SafeAreaView style={styles.safeArea}>
      ...
      <View style={styles.blueBar}>
        <TouchableOpacity onPress={() => router.push('/tema6/BusquedaBinaria')} style={styles.backButton}>
          <Ionicons name="arrow-back" size={24} color="#FFFFFF" />
        </TouchableOpacity>
        <Text style={styles.barTitle}>Tema VI: Métodos de Busqueda</Text>
      </View>
      <View style={styles.container}>
        <Text style={styles.title}>Simulador de Búsqueda Binaria</Text>
        ...
        <View style={styles.inputGroupVertical}>
          ...
          <Button
            title="BUSCAR"
            onPress={buscarBinaria}
            color="#120b8f"
            disabled={valorBusqueda.trim() === '' || isBuscando}
          />
        </View>
        ...
        {/* Mostrar mensaje explicativo de la búsqueda */}
        {mensajeExplicacion !== '' && (
          <Text style={[styles.resultado, { fontSize: 16, marginTop: 10 }]}>
            {mensajeExplicacion}
          </Text>
        )}
        ...
        <TouchableOpacity
          ...
          onPress={() => router.push('/tema6/testBB')}
        >
